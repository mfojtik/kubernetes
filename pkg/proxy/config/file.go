...
func (s ConfigSourceFile) Run() {
	...
	if err != nil {
		glog.Fatalf("Error initializing filesystem watcher for %s: %v", s.filename, err)
	}

	defer watcher.Close()

	if _, err := os.Stat(filepath.Clean(s.filename)); err == nil {
		s.updateConfig()
	}
	...
	if err := watcher.Add(filepath.Dir(filepath.Clean(s.filename))); err != nil {
