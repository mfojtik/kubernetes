...
type GeneratorArgs struct {
	...
	// These are used to deconflict the generated code you can put your fully
	// qualified package like to generate a name that doesn't conflict with your
	// group.
	PrivateNamerExceptions map[string]string
	PublicNamerExceptions  map[string]string
	PluralNamerExceptions  map[string]string
	// NamerExceptionsPath is a path to a YAML file that contains the maps for
	// above.
	NamerExceptionsPath string
}
...
func (g *GeneratorArgs) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&g.NamerExceptionsPath, "namer-exceptions-file", g.NamerExceptionsPath, "A path to file that contains public and private namer exceptions.")
}
...
// LoadExternalNamerExceptions loads the YAML file that contains namer
// exceptions and use them as a default exceptions.
func (g *GeneratorArgs) LoadExternalNamerExceptions() error {
	if len(g.NamerExceptionsPath) == 0 {
		return nil
	}
	f, err := ioutil.ReadFile(g.NamerExceptionsPath)
	if err != nil {
		return err
	}
	type namerType struct {
		PublicNamerExceptions  map[string]string `yaml:"publicNamerExceptions"`
		PrivateNamerExceptions map[string]string `yaml:"privateNamerExceptions"`
		PluralNamerExceptions  map[string]string `yaml:"pluralNamerExceptions"`
	}
	var config namerType
	if err := yaml.Unmarshal(f, &config); err != nil {
		return err
	}
	g.PublicNamerExceptions = config.PublicNamerExceptions
	g.PrivateNamerExceptions = config.PrivateNamerExceptions
	g.PluralNamerExceptions = config.PluralNamerExceptions
	return nil
}
